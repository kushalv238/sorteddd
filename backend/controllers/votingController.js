...
exports.vote = catchAsync(async(req,res,next) => {
    ...
    const locs = election.location.map(el => el.place)
    if(!locs.includes(user.location))
    {
        ...
        election.location = election.location.push(loc)
    }
    else
    {
        election.location.forEach((el, index) => {
            if (el.place === user.location) {
                const elm = {
                    place: el.place,
                    count: el.count + 1
                };
                election.location[index] = elm;
            }
        })
    }
